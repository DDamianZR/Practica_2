...
const FF_H = 80;
const GAP = 28;
const CHAIN_START_X = 64;
const CHAIN_Y = 74;
const CHIP_PAD = 18;
const DIP_Y = 18;
const LED_Y = CHAIN_Y + FF_H + 44;
...
const dipAnchor = (i: number) => ({ x: ffX(i) + 18, y: DIP_Y });
...
function FlipFlop({ index, value }: { index: number; value: Bit }) {
  ...
  return (
    <g transform={`translate(${x},${CHAIN_Y})`}>
      ...
      <text x={FF_W - 6} y={62} textAnchor="end" className="fill-muted" style={{ fontSize: 7, fontFamily: "monospace" }}>Q̄</text>
      ...
      <text x={FF_W / 2} y={FF_H + 16} textAnchor="middle" className="fill-muted" style={{ fontSize: 7, fontFamily: "monospace" }}>
        Q{index}
      </text>
    </g>
  );
...
function Led({ x, y, lit }: { x: number; y: number; lit: boolean }) {
  return (
    <g>
      <circle cx={x} cy={y} r={10} className={lit ? "fill-hi2" : "fill-lo"} filter={lit ? "url(#ffLedGlow)" : undefined} style={{ transition: "fill 120ms" }} />
      <circle cx={x} cy={y} r={10} className="fill-none stroke-line" strokeWidth={1} />
    </g>
  );
}
...
export function FlipFlopChain() {
  ...
  const chipRight = ffX(7) + FF_W + CHIP_PAD;
  const chipTop = CHAIN_Y - CHIP_PAD;
  const chipBottom = CHAIN_Y + FF_H + CHIP_PAD;

  const svgWidth = chipRight + 65;
  const svgHeight = LED_Y + 56;
  const clkBusY = chipBottom + 14;
  const clrBusY = clkBusY + 16;

  const soArrow = { x: qAnchor(7).x + 36, y: qAnchor(7).y };
  ...
  return (
    <div className="overflow-x-auto">
      <svg
        ...
        role="img"
        aria-label={`Cadena de 8 flip-flops D. Q0..Q7 = ${reg.join("")}`}
      >
        <defs>
        ...
        {/* CLK / CLR busses */}
        <line x1={24} y1={clkBusY} x2={chipRight + 24} y2={clkBusY} className="stroke-line" strokeWidth={1} />
        <motion.line
          key={`clk-${cycle}`}
          x1={24}
          y1={clkBusY}
        ...
        <line x1={24} y1={clrBusY} x2={chipRight + 24} y2={clrBusY} className="stroke-line" strokeWidth={1} />
        ...
        {Array.from({ length: 8 }, (_, i) => {
          ...
          return (
            <g key={i}>
              ...
              <text x={from.x} y={from.y - 5} textAnchor="middle" className="fill-data" style={{ fontSize: 7, fontFamily: "monospace" }}>
                D{i}={dip[i]}
              </text>
            </g>
          );
        ...
        {/* serial output stub after Q7 */}
        <Wire
          d={`M ${qAnchor(7).x} ${qAnchor(7).y} L ${soArrow.x} ${soArrow.y}`}
        ...
        {Array.from({ length: 8 }, (_, i) => {
          ...
          return (
            <g key={i}>
              <line x1={qAnchor(i).x - FF_W / 2 + 6} y1={CHAIN_Y + FF_H} x2={a.x} y2={a.y - 10} className={reg[i] === 1 ? "stroke-hi2" : "stroke-line"} strokeWidth={1.5} />
              <Led x={a.x} y={a.y} lit={reg[i] === 1} />
              <text x={a.x} y={a.y + 22} textAnchor="middle" className="fill-text" style={{ fontSize: 9, fontFamily: "monospace" }}>
                Q{i}
              </text>
            </g>
          );
              ...
              Array.from({ length: 8 }, (_, k) => {
                ...
                return (
                  <motion.circle
                    ...
                    initial={{ cx: from.x, cy: from.y, opacity: 1 }}
                    animate={{ cx: [from.x, to.x], cy: [from.y, to.y], opacity: [1, 1, 0] }}
                    transition={{ duration: 0.22, delay: k * 0.015, ease: OUT_EASE, times: [0, 0.8, 1] }}
                  />
                );
              })}
            {serialOutActive && (
              <motion.circle
                r={4}
                className="fill-hi"
                initial={{ cx: qAnchor(7).x, cy: qAnchor(7).y, opacity: 1 }}
                animate={{ cx: [qAnchor(7).x, soArrow.x + 8], cy: [qAnchor(7).y, soArrow.y], opacity: [1, 1, 0] }}
